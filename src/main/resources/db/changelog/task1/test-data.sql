...
SELECT
    ...
    CASE
        ...
        WHEN x % 10 = 2 THEN 'Кофеварка DeLonghi Magnifica ' || CHR(65 + (x % 5))
    ...
    CASE
        ...
        ELSE 'ART-' || x || '-' || SUBSTRING(CAST(RANDOM()*1000000 AS VARCHAR), 1, 6)
    END,
    CASE
    ...
    (100 + (x % 1000) * 5 + (x % 100)),
    (1 + (x % 100)),
    CURRENT_TIMESTAMP - (x % 30 || ' days')::INTERVAL,
    CURRENT_TIMESTAMP - (x % 365 || ' days')::INTERVAL
